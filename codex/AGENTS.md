...
- DO NOT rely on shell-escaped newlines with repeated `-m` flags when writing or editing a commit message. Write the full message to a temporary file and use `git commit --gpg-sign -F <file>` so paragraph breaks, backticks, and trailers are preserved exactly.
- The 72 Rule
